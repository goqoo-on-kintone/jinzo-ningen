...
const pressEditAndWaitForEditScreen = async page => {
  await page.evaluate(() => {
    window.completedHandler = event => (window.eventCompleted = true)
    kintone.events.on(['app.record.edit.show', 'app.record.index.edit.show'], window.completedHandler)
  })
  const [handle] = await Promise.all([
    page.waitForFunction('window.eventCompleted', { timeout: WAIT_FOR_ACTION_MS }),
    page.click('.gaia-argoui-app-toolbar a.gaia-argoui-app-menu-edit'),
  ])
  await page.evaluate(() => {
    window.eventCompleted = false
    kintone.events.off(['app.record.edit.show', 'app.record.index.edit.show'], window.signFinished)
  })
  return handle
}
