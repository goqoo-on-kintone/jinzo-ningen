...
const getFieldIdFromCode = async (page, fieldCode) => {
  ...
  if (!item) {
    const extItem = Object.values(fieldMap).find(field => field.label === fieldCode)
    if (extItem) {
      const { id, label, type } = extItem
      const code = extItem['var']
      console.warn(
        `Cannot find field with code: '${fieldCode}', maybe you specifed field with label: ${JSON.stringify({
          label,
          code,
          id,
          type,
      ...
      throw new Error(`Cannot find field with code: '${fieldCode}'`)
    }
  }
  return item.id
}
