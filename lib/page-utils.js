...
const isRadioButtonSelected = async (page, fieldCode, item) => {
  const inputs = await getInputs(page, fieldCode)
  return inputs.some(input => input.value === item && input.checked)
}

const isCheckBoxChecked = async (page, fieldCode, items) => {
  const result = []
  for (const item of items) {
    const checkBox = await getCheckBoxFromLabel(page, fieldCode, item)
    result.push(checkBox.checked)
  }
  return result
}

const isDropdownSelected = async (page, fieldCode, item) => {
  const fieldId = await getFieldIdFromCode(page, fieldCode)
  return page.evaluate(
    ({ fieldId, item }) => {
...
const isMultiSelectSelected = async (page, fieldCode, items) => {
  const fieldId = await getFieldIdFromCode(page, fieldCode)
  return page.evaluate(
    ({ fieldId, items }) => {
...
const getCheckBoxFromLabel = async (page, fieldCode, item) => {
  const fieldId = await getFieldIdFromCode(page, fieldCode)
...
const selectRadioButton = async (page, fieldCode, item) => {
  const fieldId = await getFieldIdFromCode(page, fieldCode)
  await page.evaluate(
    ({ fieldId, item }) => {
      const nodes = document.querySelectorAll(`.field-${fieldId} .input-radio-item-cybozu input`)
      const node = Array.from(nodes).find(node => node.value === item)
      node && node.click()
    },
    { fieldId, item }
  )
...
const selectDropdown = async (page, fieldCode, item) => {
  ...
  const dropdownItemId = await page.evaluate(selectItem => {
...
const checkCheckBox = async (page, fieldCode, itemCheckMap) => {
...
const selectMultiSelect = async (page, fieldCode, itemSelectMap) => {
  const fieldId = await getFieldIdFromCode(page, fieldCode)
